...
use albert_core::{
    ...
    MockExample, MockExampleKind, ProviderConfig, SchemaNode, SchemaNodeType, validate_value,
};
...
impl OpenAiChatAdapter {
    ...
    pub async fn generate_mock_example(
        ...
        let validation_schema = pick_validation_schema(endpoint, intent);
        let first_errors = validation_schema
            .as_ref()
            .map(|schema| validate_value(schema, &raw))
            .unwrap_or_default();
        if first_errors.is_empty() {
            return parse_response_payload(&raw, intent, None);
        }

        // Retry once with an amended prompt describing what went wrong. We
        // keep this opt-out by schema presence so endpoints without a
        // declared response shape don't loop.
        let repair_bundle = build_repair_bundle(&bundle, &first_errors);
        let repaired = match self.call_chat(&repair_bundle).await {
            Ok(value) => value,
            Err(err) => {
                // If the retry itself fails, prefer returning the first
                // generation with a warning note rather than bubbling the
                // retry error — the original payload may still be useful.
                return parse_response_payload(
                    &raw,
                    intent,
                    Some(format!(
                        "Schema validation failed; repair attempt errored: {err}"
                    )),
                );
            }
        };
        let second_errors = validation_schema
            .as_ref()
            .map(|schema| validate_value(schema, &repaired))
            .unwrap_or_default();
        if second_errors.is_empty() {
            parse_response_payload(
                &repaired,
                intent,
                Some("Repaired after one validation retry.".to_string()),
            )
        } else {
            parse_response_payload(
                &repaired,
                intent,
                Some(format!(
                    "Schema validation still failing after repair: {}",
                    second_errors.join("; ")
                )),
            )
        }
